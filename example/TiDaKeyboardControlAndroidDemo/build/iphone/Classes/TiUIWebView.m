...
- (WKWebView *)webView
{
  if (_webView == nil) {
    ...
    [controller addUserScript:[self userScriptKeyboardcontroldemoInjectionForAppEvent]];
...
- (WKUserScript *)userScriptKeyboardcontroldemoInjectionForAppEvent
{
  if (_pageToken == nil) {
  ...
  NSString *_keyboardcontroldemo = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium"];
  ...
  NSString *sourceString = [NSString stringWithFormat:source, _keyboardcontroldemo, _pageToken, baseInjectScript];
...
- (WKUserScript *)userScriptKeyboardcontroldemoJSEvaluationFromString:(NSString *)string
{
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(nonnull WKNavigationAction *)navigationAction decisionHandler:(nonnull void (^)(WKNavigationActionPolicy))decisionHandler
{
  ...
  if ([allowedURLSchemes containsObject:navigationAction.request.URL.scheme]) {
    if ([[UIApplication sharedApplication] canOpenURL:navigationAction.request.URL]) {
      // Event to return URL to Keyboardcontroldemo in order to handle OAuth and more
