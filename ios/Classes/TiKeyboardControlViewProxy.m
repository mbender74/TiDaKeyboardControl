   ...
   // Re-apply toolbar position when keyboard will show with correct animation.
    ...
    if (!CGRectIsEmpty(lastInputAccessoryViewFrame) && nativeScrollView != nil) {
        ...
        // Animate toolbar with spring physics to match keyboard
        {
            ...
            CGFloat delay = keyboardTransitionDuration * MAX(0, delayRatio) * 0.27;  // 27% des Verhältnisses
            ...
            NSLog(@"[TOOLBAR] Spring anim: toolbarDist=%.1f keyboardDist=%.1f delay=%.3fs duration=%.3fs damping=0.9", 
